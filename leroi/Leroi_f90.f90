!   PROGRAM Leroi          Version 7.1.0    11 November 2007
!   ======= =====          =============    ================
!
!
...
!     ==============================
!
!   1. Correction of error in Utem presentation
!
!   2. DATA_FLOOR is now used for all data weighting instead of N0STAT and N0PTS (Changed Leroi.inv input)
!
!   3. Plate orientation angles have been stabilised using default bounds.
!   
!   4. Moving loop option now requires z coordinate for loop.  (Changed Leroi.cfl input)
!
!
!     P223F Features for Leroi 7.0.5
!     ==============================
!
!   1. Inversion capability has been included.
!      The user must choose which components are to be modelled or inverted for
!      magnetic dipole or point electrode through the choice of a new input
!      variable, CMP.  (Input format change if RX_TYPE = 1 or 3.)
!      This does not apply to magnetotelluric computations.
!
!   2. There is no restriction on the number of layers above basement.
!
!   3. Plunge has been implemented.  Plate orientation is now described
!      by 3 angles: Dip Azimuth, Dip and Plunge.  There is no restriction
!      on the number of plates.
!
!   4. In this version, all plates must be contained within the basement. In the
!      event that plunge would require part of the plate to protrude above
!      basement, Leroi moves the plate down.
!
!   5. The default spectrum for the 3D part is now from 0.1 KHz 100 KHz.
!      This can be changed by setting TDFD = 0 in  RECORD 2 and entering
!      new spectrum parameters in RECORD 2.1
!
!   6. A magnetotellurics option has been included.
!
!   7. Sources and receivers can be in any layer.  Closed and open loops must be
!      horizontal; ie, all vertices must have the same elevation (depth).
!
!   8. There are four source types: closed loops, open loops (straight
!      grounded wire or multiple vertex loop grounded at each end of the open
!      side), magnetic dipoles and plane wave.
!      Only one source type can be used for a single model run.
!
!   9. There are three types of receivers: magnetic dipole (B or dB/dt),
!      electric dipole (voltage) or point electric field.  All three may be used
!      during a single model run but each line must have only one receiver type.
!
!  10. There are six survey types including a magnetotellurics option.
!      Using the variable, ISYS, the user can choose standard, Sampo or Utem
!      processing.  For downhole receives, components can be defined using the
!      U-V-A conventions, the Utem convention or X-Y-Z referenced to a flat earth
!      rather than a dipping borehole.
!
!  11. Leroi is now based on lines rather than transmitter and receiver groups
!      A line can be either moving transmitter with a single constant-offset receiver
!      or a single transmitter with several receivers.
!
!      A moving transmitter with three, constant-offset receivers will produce three
!      output lines for every transmitter line.
!
!  12. Revised SET_OUTPUT and WRITE routines for _TD, _FD and _MT
!
!  13. Arrays BFDL, BTDL and ZMT have been replaced by BFTL
!
!  14. The single plate Green's function computation routines to have been
!      changed to full numerical Gauss-Legendre integration, replacing the
!      analytic integrations down dip.  In addition to improving geometric
!      flexibility, this has resulted in marginal improvements in both speed
!      and accuracy.
!
!  15. The inter-plate Green's functions have been phrased more coherently
...
!
!  16. The exact "at a point" integrals for primary fields, single and
!      inter-plate Green's functions and MGT and EGT_OUT have all been
!      replaced by a 2D interpolation net with cubic spline interpolation in
!      the horizontal direction and non-uniform 4 point Lagrange interpolation
!      in the vertical direction.
!
!  17. Revised WRITE_TD & WRITE_FD (including subsidiary routines)
!      Percent target effect eliminated.
!
!  18. Corrected Utem processing (reversed channels)
!
!
!================
!   DESCRIPTION |
...
!  in a horizontally layered host.  At present, the target(s) are restricted to
!  be in the basement of a multi-layered host.  The top layer surface is defined.
!
...
!  borehole systems as well as magnetotellurics.  It can be used for plane wave
!  sources, magnetic dipole, closed loop or dipole transmitters.  It can be used
...
!      14       NW1      Leroi.mv1      Inversion plotting output   x
!      14       NW1      Leroi.mf1      Forward model output                x
!
!------------------------------------------------------------------------
!
...
!
!##      ======================================================================
!##      FOR MT inversion, skip this section and go to MT Inversion Description
!##      ======================================================================
!
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!=================================================================================!
!=                                                                               =!
!=      INVERSION DATA SPECIFICATION IF NOT MAGNETOTELLURICS:                    =!
!=                                                                               =!
!=                                                                               =!
...
!
!===========================================================================
!   RECORDS 1 to 4 below apply for SURVEY_TYPE = 1, 2, 3, 4 or 5
!   They do NOT apply for MT inversion
!
!   For MT inversion, skip RECORDS 1 to 4 and go to RECORD 5
!===========================================================================
!
...
!==========================================================
!                                                         =
!     END DATA ENTRY DESCRIPTIOM for non-MT inversion     =
!                                                         =
!**********************************************************
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!=================================================================================!
!=                                                                               =!
!=      INVERSION DATA SPECIFICATION FOR MAGNETOTELLURICS                        =!
!=                                                                               =!
!=                                                                               =!
!=      BEGIN DESCRIPTON FOR Leroi.inv (Logical unit NRI = 13)                   =!
!=      ------------------------------------------------------                   =!
...
!=================================================================================!
!
!  Leroi assumes that the MT data, 1 record for each of NERX stations
!  will be entered in RECORD 9 below the following order:
!
!  If IPM = 0 in RECORD 6 of Leroi.cfl
!  ------------------------------------
!    ZXX_inphase(1:NFRQ), ZXX_quad(1:NFRQ), ZXY_inphase(1:NFRQ), ZXY_quad(1:NFRQ),
!    ZYX_inphase(1:NFRQ), ZYX_quad(1:NFRQ), ZYY_inphase(1:NFRQ), ZYY_quad(1:NFRQ)
!
!  If IPM = 1 in RECORD 6 of Leroi.cfl
!  ------------------------------------
!    ZXX_phase(1:NFRQ), ZXX_ampl(1:NFRQ), ZXY_phase(1:NFRQ), ZXY_ampl(1:NFRQ),
!    ZYX_phase(1:NFRQ), ZYX_ampl(1:NFRQ), ZYY_phase(1:NFRQ), ZYY_ampl(1:NFRQ)
!
!  In the event that one component is missing as determined by RECORD 5 below, the
!  order of the remaining comp[onents should be kept.
!
!** RECORD 1: KZXX, KZXY, KZYX, KZYY
!
!       KZXX = 1 : Leroi.inv will contain ZXX data
!            = 0 : Leroi.inv will NOT contain ZXX data
!
!       KZXY, KZYX and KZYY are similarly defined for ZXY, ZYX and ZYY data
!
!
!**RECORD 2.1: DATA_FLOOR_I (I) I = 1 to 2*NFRQ)
!
!          Enter NFRQ Data Floors for the Inphase (or phase if IPM = 1) component
!          followed by NFRQ Data Floors for the quadrature (or magnitude if IPM = 2) component
!
!
!             DATA ENTRY   (read from Leroi.inv)
!             ==========
!
!    DO for each receiver J of NERX stations
!**  RECORD 3.J: KSTAT(J), ZXX, ZXY, ZYX, ZYY
!
!       KSTAT(I,J) = station index of receiver I (sequential from 1 to NERX
!
!       ZXX, ZXY, ZYX, ZYY are entered as per description before RECORD 1,
!       depending upon the values of KZXX, KZXY, KZYX, KZYY and IPM
!
!==============================
...
! -----------------------------
 ...
 INTEGER FVN,NR,NW,ND,NLG,NRI,NW1,MSG,MXERR,DO3D,TDFD,IPPD,STEP,NSX,PRFL,ISTOP,KRXW, &
         ...
         MRXTX,MRXL,NLITH,NPULS,NTYRP,NTYPLS,NPPD,MD1,MD2,NERX,NHRX,IPM,NHID
 ...
 DATA PVC, FVN /'Leroi - Version 7.1.0   11 November 2007',690/
 DATA LTXT     /'---------------------------------------'/
...
! ------------------------------
 ...
 INTEGER,ALLOCATABLE, DIMENSION(:) :: NA,NB,NCELL2,LITHP,KPCT
 REAL CELLW
...
!      = 1 for magnetic dipole; = 2 for electric dipole
 ...
 1 FORMAT (                                                                    &
  ...
  /T7,'#',T25,A,T76,                                                       '#' &
  /T7,'#                                                                    #' &
  /T7,'#      Develped by:  Art Raiche                                      #' &
  /T7,'#              For:  AMIRA project P223F                             #' &
  /T7,'#                                                                    #' &
  /T7,'######################################################################'/)
 2 FORMAT (T1,79('-'))
...
!  Start reading plate variables if DO3D > 0
 ...
 ALLOCATE (LITHP(NPLT),SIG_T(NPLT),CHRGP(NPLT),CTAUP(NPLT),CFREQP(NPLT),PLTOP(NPLT),    &
           PLTOPD(NPLT),XCNTR(NPLT),YCNTR(NPLT),XCNTRD(NPLT),YCNTRD(NPLT),PLNGTH(NPLT), &
           PLWDTH(NPLT),DZM(NPLT),PLAZM(NPLT),DIP(NPLT),PLDIP(NPLT),PLG(NPLT),          &
           PLUNJ(NPLT),NA(NPLT),NB(NPLT),NCELL2(0:NPLT),DA(NPLT),DB(NPLT))

 LITHP=0; SIG_T=-1.; CHRGP=0; CTAUP=0; CFREQP=0; PLTOP=0; XCNTR=0; YCNTR=0;  PLNGTH=0;
 PLWDTH=0; DZM=0; DIP=0; PLG=0
 NA=0; NB=0; NCELL2=0; DA=0; DB=0; MXAB=4
...
! a right-handed system with Z downwards.
 ...
 DO JP = 1, NPLT
   ...
   MXAB = MAX (MXAB, MXABT)
 END DO
 IF (INVERT) MXAB = 2 * MXAB
 ALLOCATE (XCELL(MXAB,NPLT),YCELL(MXAB,NPLT),ZCELL(MXAB,NPLT))
...
! Write the all the data not weighted to zero.
 ...
 SUBROUTINE PREPARE_INVRT_MT_DATA
!--------------------------------

 INTEGER MCHNL,NKMP,MD,KMPI,NDT,JC,JR,JF,J1,JRD
 REAL, ALLOCATABLE,DIMENSION (:) :: DATA_FLOOR,QDATA
 CHARACTER (LEN=1) TCHR
 CHARACTER (LEN=3) ZCH(4)
 DATA ZCH /'ZXX','ZXY','ZYX','ZYY'/
...
!  MCMP = 4
!
!  RDATA & RWTS are data and weights in array form (MCHNL, NERX)
!  XDATA is the  data in column form  (MCHNL * NERX)
!  RWTS is now restricted to integer values of 0 or 1 (reject or accept)
!  Note that data are normalised so that balance weighting is unnecessary.
!
!  XMODL contains model results in column form (MCHNL * NERX)
!  NRX = 1 for TD & NFRQ for FD
!
!  For TD, RDATA is ordered as vertical components for all delay times
!  followed by all in-line components, followed by all transverse
...
!  First skip over alL comment lines
 ...
 MCHNL = 2*NFRQ
 ALLOCATE (QDATA(MCHNL*4),DATA_FLOOR(MCHNL))
 ...
 READ(NRI,*) KMPMT(1:4)
 WRITE(NW,1) KMPMT(1:4),CMPMT(1:4)
 KMPI = 1000*KMPMT(1) + 100*KMPMT(2) + 10*KMPMT(3) + KMPMT(4)
 SELECT CASE (KMPI)
 CASE (1111)
   NKMP = 4
 CASE (0111, 1011, 1101, 1110)
   NKMP = 3
 CASE (0011, 0101, 0110, 1001, 1010, 1100)
   NKMP = 2
 CASE (1000, 0100, 0010, 0001)
   NKMP = 1
 END SELECT
...
!  Arrange data such that ZXX, ZXY, ZYX, ZYY are in their correct positions
!  even if some of them are missing

   IF (KMPMT(1) == 1) THEN
     RDATA(1:MCHNL,JR,1,1) = QDATA(1:MCHNL)
   ELSE
     RWTS(1:MCHNL,JR,1,1) = 0
   END IF

   DO J1 = 1,4
     IF (CMPMT(J1) == 0) THEN
       RWTS(1:MCHNL,JR,J1,1) = 0
     ELSE
       IF (KMPMT(J1) == 0) THEN
         CALL WRITE_LOG_FILE(NLG,100,MXERR,2)
         WRITE(NLG,2) ZCH(J1)
   ...
   SELECT CASE (KMPI)
   CASE (1111)
     RDATA(1:MCHNL,JR,2,1) = QDATA(MCHNL+1:2*MCHNL)
     RDATA(1:MCHNL,JR,3,1) = QDATA(2*MCHNL+1:3*MCHNL)
     RDATA(1:MCHNL,JR,4,1) = QDATA(3*MCHNL+1:4*MCHNL)
   CASE (1110)
     RDATA(1:MCHNL,JR,2,1) = QDATA(MCHNL+1:2*MCHNL)
     RDATA(1:MCHNL,JR,3,1) = QDATA(2*MCHNL+1:3*MCHNL)
     RWTS(1:MCHNL,JR,4,1) = 0
   CASE (1101)
     RDATA(1:MCHNL,JR,2,1) = QDATA(MCHNL+1:2*MCHNL)
     RDATA(1:MCHNL,JR,4,1) = QDATA(2*MCHNL+1:3*MCHNL)
     RWTS(1:MCHNL,JR,3,1) = 0
   CASE (1011)
     RDATA(1:MCHNL,JR,3,1) = QDATA(MCHNL+1:2*MCHNL)
     RDATA(1:MCHNL,JR,4,1) = QDATA(2*MCHNL+1:3*MCHNL)
     RWTS(1:MCHNL,JR,2,1) = 0
   CASE (0111)
     RDATA(1:MCHNL,JR,2,1) = QDATA(1:MCHNL)
     RDATA(1:MCHNL,JR,3,1) = QDATA(MCHNL+1:2*MCHNL)
     RDATA(1:MCHNL,JR,4,1) = QDATA(2*MCHNL+1:3*MCHNL)
   CASE (1100)
     RDATA(1:MCHNL,JR,2,1) = QDATA(MCHNL+1:2*MCHNL)
     RWTS(1:MCHNL,JR,3:4,1) = 0
   CASE (1010)
     RDATA(1:MCHNL,JR,3,1) = QDATA(MCHNL+1:2*MCHNL)
     RWTS(1:MCHNL,JR,2,1) = 0
     RWTS(1:MCHNL,JR,4,1) = 0
   CASE (1001)
     RDATA(1:MCHNL,JR,4,1) = QDATA(MCHNL+1:2*MCHNL)
     RWTS(1:MCHNL,JR,2:3,1) = 0
   CASE (0110)
     RDATA(1:MCHNL,JR,2,1) = QDATA(1:MCHNL)
     RDATA(1:MCHNL,JR,3,1) = QDATA(MCHNL+1:2*MCHNL)
     RWTS(1:MCHNL,JR,4,1) = 0
   CASE (0101)
     ...
     RWTS(1:MCHNL,JR,3,1) = 0
   CASE (0011)
     RDATA(1:MCHNL,JR,3,1) = QDATA(1:MCHNL)
     RDATA(1:MCHNL,JR,4,1) = QDATA(MCHNL+1:2*MCHNL)
     RWTS(1:MCHNL,JR,2,1) = 0
   CASE (0100)
     RDATA(1:MCHNL,JR,2,1) = QDATA(1:MCHNL)
   ...
   CASE (0010)
     ...
     RWTS(1:MCHNL,JR,4,1) = 0
   CASE (0001)
     RDATA(1:MCHNL,JR,4,1) = QDATA(1:MCHNL)
     RWTS(1:MCHNL,JR,2:3,1) = 0
   END SELECT

! Set RWTS = 0 for unused data components.

   DO JC = 1,4
     DO JF = 1,MCHNL
       IF (ABS (RDATA(JF,JR,JC,1)) < DATA_FLOOR(JF)) RWTS(JF,JR,JC,1) = 0
       RDATA(JF,JR,JC,1) = RDATA(JF,JR,JC,1) * RWTS(JF,JR,JC,1)
     END DO
   END DO
 END DO
 DEALLOCATE (QDATA)

!  Write revised inversion data

 KPRT = -1
 CALL WRITE_MT (NW,NW1,KPRT,NFRQ,FREQ,NERX,TITLE,MTPLT,IPM,BFTL,RDATA,RWTS)
 ...
 DO JC = 1,4
   DO JR = 1,NERX
     DO JF = 1,MCHNL
       IF (RWTS(JF,JR,JC,1) > 0) THEN
         NDATA = NDATA + 1
         QDATA(NDATA) = RDATA(JF,JR,JC,1)
       END IF
     END DO
   END DO
 END DO
 ...
 DEALLOCATE (QDATA)
  ...
  1 FORMAT(/T2,'=======================================' &
           /T3,'Inversion controls for MT:  KMPMT =',4I3,4X,'CMPMT =',4I3)
  2 FORMAT(/T3,A,' data is specified by Leroi.cfl for inversion but is not contained in the Leroi.inv data.')
  4 FORMAT(/T8,'Frequency Domain Data Floors'/T8,'----------------------------' &
          //T8,'Freq     In-phase   Quadrature'/)
  6 FORMAT(I4,F9.0,2G12.4)
 17 FORMAT(/T3,'Weighted data'/)
 20 FORMAT(//T3,'NDATA =',I5,' data points.' &
            /T3,'This excludes components not specified by CMP and data from the above set' &
            /T3,'that hasbeen weighted to zero using data floors.')

   END SUBROUTINE PREPARE_INVRT_MT_DATA
...
!  Set up and print 3D model.  Convert angles to radians
...
! cells in different plates.  Convert angles to radians
 ...
 24 FORMAT(//T3,'Plot points for receivers on Line',I7, &
           //T13,'East        North     Elev'          &
            /T13,'----        -----     ----' /I4,2F13.1,F9.1)
 25 FORMAT (//11X,'Target Cell Centre Locations for Plate',I2         &
             /11X,'----------------------------------------'          &
            //'   Dip   Strike                      '                 &
 ...
 39 FORMAT (//T3,'********************************************************'     &
             ...
             /T3,'********************************************************')
 40 FORMAT(//T3,'Transformed Input Data for Plate',I3 &
            /T3,'-----------------------------------')
 ...
 44 FORMAT(/T3,'Plate',I3,' is discretised into:', &
           /T6,I3,' cells of length',F7.2,' metres along strike and' &
           /T6,I3,' cells of length',F7.2,' metres down dip.')
...
!  This is not the case for Sampo or MT.
 ...
 CASE (260)            ! FD magnetotellurics : Inphase & Quadrature
   WRITE(NW1,2) TRIM (ADJUSTL(QL)), HID, (ZXXR,JF,JF=1,NFRQ), (ZXXQ,JF,JF=1,NFRQ), (ZXYR,JF,JF=1,NFRQ), (ZXYQ,JF,JF=1,NFRQ), &
                                         (ZYXR,JF,JF=1,NFRQ), (ZYXQ,JF,JF=1,NFRQ), (ZYYR,JF,JF=1,NFRQ), (ZYYQ,JF,JF=1,NFRQ)

 CASE (261)            ! FD magnetotellurics : Phase and Magnitude
   WRITE(NW1,2) TRIM (ADJUSTL(QL)), HID, (ZXXP,JF,JF=1,NFRQ), (ZXXM,JF,JF=1,NFRQ), (ZXYP,JF,JF=1,NFRQ), (ZXYM,JF,JF=1,NFRQ), &
...
! Put arrays in body centred system.
 ...
 IF (INVERT) THEN
   ...
   OPEN(NW1,FILE = 'Leroi.mv1',STATUS = 'REPLACE')
...
!  For time-domain, set up frequencies, interpolation times
 ...
 IF (INVERT) THEN
   CALL NLSQ2 (NPAR,NDATA,XDATA,RDATA,RWTS,PCTCNV,NDSTP,KPCT,CXPAR,ELAS,LBND,UBND,MAXITS,INVPRT, &
               NW,NW1,TITLE,SURVEY_TYPE,MT,SOURCE_TYPE,NLINES,MLINES,LINE,HEADER_ID,IPLT,MTPLT,  &
               YXZPLT,MCMP,CMP,MRXL,MCHNL,NCHNL,NFRQ,NFRQHS,TDFD,STEP,FREQ,FRQHS,NSX,SWX,SWY,    &
               NPULS,PULSE,NTYPLS,NTYRP,TRP,TOPN,TCLS,NFT,CURNT,ISYS,SVAZM,UNITS,RX_TYPE,IDH,    &
               IPM,IDHE,MXTX,NTX,SXN,SXE,SXZ,SXDIP,SXAZM,MXVRTX,NVRTX,NRX,LNTR,NRXTX,MRXTX,      &
...
!  dipole moments happens in the output routines.
     ...
     CALL LEROI_3D (NFRQ,FREQ,SOURCE_TYPE,NTX,MXVRTX,NVRTX,SXN,SXE,SXZ,SXDIP,SXAZM,NRXTX,   &
                    MRXTX,RXID,MQVR,MXRS,XRXTX,YRXTX,ZRXTX,NLYR,THKD,RES,RMUD,REPS,CHRG,    &
                    CTAU,CFREQ,NPLT,MXAB,CELLW,PLNGTH,PLWDTH,XCNTR,YCNTR,PLTOP,PLAZM,PLDIP, &
                    PLUNJ,SIG_T,CHRGP,CTAUP,CFREQP,MXRHO,RHOTRP,INVERT,BFD_SCAT)
...
!  Writes frequency-domain output in profile form.  IPPM = format control
 ...
 SUBROUTINE WRITE_MT (NW,NW1,KPRT,NFRQ,FREQ,NERX,TITLE,MTPLT,IPM,BFTL,RDATA,RWTS)
!-------------------------------------------------------------------------------

!  Writes Mamagnetotelluric impedances to units NW & NW1
!
!*** Called by: MAIN
!***     Calls:
!
!   NW            : verbose print output unit number
!   NW1           : plot output unit number
...
!    JZ = 1 to 4 represents ZXX, ZXY, ZYX and ZYY respectively
 ...
 IMPLICIT NONE
 INTEGER, PARAMETER :: QL=SELECTED_REAL_KIND(12,80)
 INTEGER NW,NW1,KPRT,NERX,NFRQ,MCHNL,IPM,HID,JF,JE,JC
 ...
 MCHNL = 2*NFRQ
 DO JE = 1,NERX
   WRITE(NW1,16) JE,MTPLT(1:2,JE), (BFTL(1:MCHNL,JE,JC,1), JC=1,4)
 END DO
...
!    JR = NHRX+1 TO NHRX + NERX : E firld receivers
 ...
 IMPLICIT NONE
 INTEGER, PARAMETER :: QL=SELECTED_REAL_KIND(12,80), NTX=2
...
!*****************************
 ...
 SUBROUTINE LEROI_3D (NFRQ,FREQ,SOURCE_TYPE,NTX,MXVRTX,NVRTX,SXN,SXE,SXZ,SXDIP,SXAZM,NRXTX,   &
                      MRXTX,RXID,MQVR,MXRS,XRXTX,YRXTX,ZRXTX,NLYR,THKD,RES,RMUD,REPS,CHRG,    &
                      CTAU,CFREQ,NPLT,MXAB,CELLW,PLNGTH,PLWDTH,XCNTR,YCNTR,PLTOP,PLAZM,PLDIP, &
                      PLUNJ,SIG_T,CHRGP,CTAUP,CFREQP,MXRHO,RHOTRP,INVERT,BFD_SCAT)
!----------------------------------------------------------------------------------------------
...
!
 ...
 IMPLICIT NONE
 INTEGER, PARAMETER :: QL=SELECTED_REAL_KIND(12,80), NMG1=3, NQS=1000     !  Number of points for MGT integration
 COMPLEX, PARAMETER :: ZERO=(0.,0.)
 INTEGER NW,NFRQ,SOURCE_TYPE,NTX,MXVRTX,NVRTX(NTX),NRXTX(NTX),MRXTX,NRS(MRXTX,NTX),MXRS, &
         RXID(MRXTX,NTX),MQVR,MXRHO,MXCL2,NMGP,NRPRM,NREGT,NRMGT,NPPD,ACC,NZ1,NZ2,IFIN,J1,JF,JS,JR
 REAL FRQ,FREQ(NFRQ)
 REAL, DIMENSION(NTX) :: SXDIP,SXAZM,SXZ
 REAL, DIMENSION (MXVRTX,NTX) :: SXN,SXE
 ...
 REAL, ALLOCATABLE, DIMENSION(:,:) :: XMG,YMG,ZMG
 COMPLEX BFD_SCAT(NFRQ,MRXTX,NTX,3)
 LOGICAL MDRX,EDRX,QUIT,MT,DCMP_FAIL,INVERT

! Model dependent variables

 INTEGER NLYR,NPLT,MXAB,NCELL2(0:NPLT),ICCL(NLYR)
 INTEGER, DIMENSION(NPLT) :: NA,NB,ICCP
 REAL CELLW,DPTHB,RHOTRP(MXRHO),ZMIN,ZMAX,ZMIN2,ZMAX2,T1,SKN,SPAN,DEL0,ZV1(NQS),ZV2(NQS)
 REAL(KIND=QL) RMUD(0:NLYR)
 REAL(KIND=QL), DIMENSION(NLYR) :: THKD,DPTHL
 REAL, DIMENSION (NLYR) :: RES,REPS,CTAU,CFREQ,CHRG
 REAL, DIMENSION(NPLT) :: SIG_T,CHRGP,CTAUP,CFREQP,XCNTR,YCNTR,PLTOP,SHIFT, &
                          PLWDTH,PLNGTH,PLAZM,PLDIP,PLUNJ,DA,DB,WMG
 REAL, DIMENSION(MXAB,NPLT) :: XCELL,YCELL,ZCELL
 COMPLEX(KIND=QL), DIMENSION(NLYR) :: KSQL, SIGL
 COMPLEX, DIMENSION(NPLT) :: KSQT, SIGT
 COMPLEX, DIMENSION(2,MXAB,NTX,NPLT) :: J_SCAT,E_PRYM

! Construct plate representation

 NW = 100               ! Don't print warniong messages from SET_CELLS
 DPTHL = 0._QL
 DO J1 = 2,NLYR
   DPTHL(J1) = DPTHL(J1-1) + THKD(J1-1)  ! Depth to top of layer
 END DO
 DPTHB = REAL (DPTHL(NLYR))              ! Depth to basement

 CALL SET_CELLS (NW,DPTHB,NPLT,MXAB,CELLW,PLNGTH,PLWDTH,XCNTR,YCNTR,PLTOP,PLAZM,PLDIP, &
                 PLUNJ,NA,NB,DA,DB,MXCL2,NCELL2,XCELL,YCELL,ZCELL,ZMIN,ZMAX,SHIFT)

 MT = .FALSE.
 IF (SOURCE_TYPE == 5) MT = .TRUE.
...
! Set up uniform integration for magnetic field computation
 ...
 CALL SET_MAX_INTRP (MT,MXRHO,RHOTRP,MXAB,NPLT,NA,NB,DA,PLNGTH,XCELL,YCELL,NTX,MXVRTX, &
                     NVRTX,SXN,SXE,MRXTX,NRXTX,MXRS,NRS,XRS,YRS,NRMGT,NRPRM,NREGT)

!  Set depth interpolation

 DO J1 = 2,NPLT
   T1 = PLTOP(J1) + PLWDTH(J1) * SIN (PLDIP(J1))
   ZMAX = MAX (ZMAX,T1)
 END DO
 ZMIN2 = 2.* (ZMIN - DPTHB)
 ZMAX2 = 2.* (ZMAX - DPTHB)
...
! scattering matrices and scattering currents for each plate individually.
   ...
   ZV1 = 1.E5
   ZV2 = 1.E5
   CALL SET_Z (ACC,NPPD,ZMIN,ZMAX,SPAN,DEL0,NQS,ZV1,NZ1)      ! Used for MGT & PRM routines
   IF (ZV1(1) < DPTHB) THEN
     ZV1(2) = (ZMIN + DPTHB) / 2.
     ZV1(1) = (ZV1(2) + DPTHB) / 2.
   END IF

   CALL SET_Z (ACC,NPPD,ZMIN2,ZMAX2,SPAN,DEL0,NQS,ZV2,NZ2)    ! Used for EGT routines
...
!  Compute the layered earth electric fields on the target, E_PRYM.
   ...
   IF (SOURCE_TYPE == 3) THEN   ! Magnetic dipole
     CALL PRM_BOSS_MD (FRQ,NTX,SXN,SXE,SXZ,SXDIP,SXAZM,NRPRM,RHOTRP,NLYR,THKD,DPTHL, &
                       RMUD,SIGL,KSQL,NPLT,PLAZM,PLDIP,PLUNJ,MXAB,NB,NA,XCELL,YCELL, &
                       ZCELL,NZ1,ZV1,E_PRYM)


   ELSE IF (SOURCE_TYPE == 5) THEN   ! plane wave vertically incidence
     CALL PRM_BOSS_MT (NLYR,THKD,DPTHL,RMUD,KSQL,NPLT,PLAZM,PLDIP,PLUNJ,MXAB,NB,NA, &
                       ZCELL,NZ1,ZV1,E_PRYM)

   ELSE                          ! Open or closed loop
     CALL PRM_BOSS_LP (SOURCE_TYPE,NTX,MXVRTX,NVRTX,SXN,SXE,SXZ,NRPRM,RHOTRP,NLYR,  &
                       THKD,DPTHL,RMUD,SIGL,KSQL,NPLT,PLAZM,PLDIP,PLUNJ,MXAB,NB,NA, &
                       XCELL,YCELL,ZCELL,NZ1,ZV1,E_PRYM)
   END IF

!  Set up scattering matrix SCAT_MTRX as an LU decomposition.

   CALL SCAT_MTRX_BOSS (NLYR,THKD,DPTHB,RMUD,SIGL,KSQL,NPLT,MXAB,NCELL2,MXCL2,NA,NB, &
                        DA,DB,SIGT,KSQT,PLTOP,PLAZM,PLDIP,PLUNJ,XCELL,YCELL,ZCELL,   &
                        NZ2,ZV2,NREGT,RHOTRP,NTX,E_PRYM,J_SCAT,DCMP_FAIL)
   IF (DCMP_FAIL) THEN
     WRITE(*,3)
     STOP
   END IF
...
!  Compute BFD_SCAT, the scattered frequency-domain magnetic fields.
   ...
   CALL SCAT_EM (JF,NFRQ,NPLT,MXAB,NA,NB,PLAZM,PLDIP,PLUNJ,XCELL,YCELL,ZCELL,NMGP,XMG,YMG,ZMG,WMG, &
                 NLYR,THKD,DPTHL,RMUD,SIGL,KSQL,NTX,MRXTX,NRXTX,RXID,MXRS,NRS,XRS,YRS,ZRXTX,WTRS,  &
                 EDCS,EDSN,NRMGT,RHOTRP,NZ1,ZV1,MDRX,EDRX,J_SCAT,BFD_SCAT,QUIT)

   IF (QUIT) EXIT    !  If scattered fields are negligible, end frequency stepping

 END DO FREQUENCY_LOOP
 DEALLOCATE (XMG,YMG,ZMG)
...
! -------------------------------------------------------------------------
 ...
 COMPLEX, PARAMETER :: ZERO=(0.,0.)
 INTEGER NAL,NALP,NBL,NAB,NREGT,NZ2,J1,J2,KB,KA,JB,JA,JB_KB,JA_KA,JAB,KAB
 ...
 DO KB = 1, NBL                                  ! receiver cell row loop
   DO JB = KB, NBL                               ! source cell row loop
     DO J1 = 1, NGL
       ...
       DO KA = 1, NALP                           ! receiver cell column loop

!     JA = KA.  Use symmetry wrt XI.
         ...
         DO J2 = 1,NGL
           ...
           GXYZ(1,1) = (2.*XBARSQ - 1.) * GC4 + YBARSQ * GC1             ! Sxx
           GXYZ(2,2) = (2.*YBARSQ - 1.) * GC4 + XBARSQ * GC1             ! Syy
           GXYZ(3,3) = GC3                                               ! Szz
           ...
           GXYZ(1,1) = (2.*XBARSQ - 1.) * GC5 - XBARSQ * GC2             ! Vxx
           GXYZ(2,2) = (2.*YBARSQ - 1.) * GC5 - YBARSQ * GC2             ! Vyy
           GXYZ(3,3) = -GC2                                              ! Vzz
         ...
         DO JA = KA+1, NAL                       ! receiver cell column loop
           ...
           DO J2 = 1, NGL
             ...
             GXYZ(2,2) = (2.*YBARSQ - 1.) * GC4 + XBARSQ * GC1             ! Syy
             GXYZ(3,3) = GC3                                               ! Szz
             IF (KB /= JB) GXYZ(1,2) = XBAR * YBAR * (2.* GC4 - GC1)       ! Sxy
             ...
             GXYZ(2,2) = (2.*YBARSQ - 1.) * GC5 - YBARSQ * GC2             ! Vyy
             GXYZ(3,3) = -GC2                                              ! Vzz
             IF (KB /= JB) GXYZ(1,2) = XBAR * YBAR * (2.* GC5 - GC2)       ! Vxy
...
!  If ROT = 0, fill in KA-JA values for JA >= KA
...
!     JUMP - keep integrating if false.
 ...
 COMPLEX (KIND=QL) LMBSQ,SN,EHRI(NRHO,6),KER(JNLO-NRHO:JNHI,6),G,ETA,TXP2,TMP(6)
...
! rotate them into the plate system.
         ...
         GXYZ = ZERO
         GXYZ(1,1) = SXXD + (2.*XBARSQ - 1.)  * Q4 + YBARSQ * Q1
         GXYZ(2,2) = SXXD + (2.*YBARSQ - 1.)  * Q4 + XBARSQ * Q1
         GXYZ(3,3) = SXXD + Q3
...
! rotate them into the plate system.
         ...
         GXYZ(1,1) = VXXD + KSQN * GXYZ(1,1) + (2.*XBARSQ - 1.) * Q5 - XBARSQ * Q2
         GXYZ(2,2) = VYYD + KSQN * GXYZ(2,2) + (2.*YBARSQ - 1.) * Q5 - YBARSQ * Q2
         GXYZ(3,3) = VZZD + KSQN * GXYZ(3,3) - Q2
         GXYZ(1,2) = VXYD + KSQN * GXYZ(1,2) + XBAR * YBAR * (2 * Q5 - Q2)
         GXYZ(2,1) = GXYZ(1,2)
         GXYZ(1,3) = VZXD + XBAR * Q6   ! These are the GTE computed by switching
         GXYZ(3,1) = VZXD - XBAR * Q6   ! the "source" and "receiver" points.
         GXYZ(2,3) = VYZD + YBAR * Q6   ! Hence the reversed negative sign that
         GXYZ(3,2) = VYZD - YBAR * Q6   ! affects only the xz, zx, zy,yz components.
...
!
 ...
 SUBROUTINE EGT_OUT_BOSS (NPLT,MXAB,NA,NB,PLAZM,PLDIP,PLUNJ,XCELL,YCELL,ZCELL,NLYR,THKD,DPTHL, &
                          RMUD,SIGL,KSQL,NTX,MRXTX,NRXTX,RXID,MXRS,NRS,XRS,YRS,ZRXTX,WTRS,     &
                          NRMGT,RHOTRP,NZ1,ZV1,NMGP,XMG,YMG,ZMG,WMG,HA,HB)
!--------------------------------------------------------------------------------------------
...
!  integrated over the area of individual cells.
 ...
 INTEGER, PARAMETER :: QL=SELECTED_REAL_KIND(12,80)
 COMPLEX, PARAMETER :: ZERO=(0.,0.)
 INTEGER NINTG,NPLT,MXAB,NLYR,RXLYR,KFG,NTX,MRXTX,NRXTX(NTX),MXRS,NRS(MRXTX,NTX),NRMGT,NZ1,NMGP, &
         JA,JB,JP,JR,JR1,JS,JAB,J1,I1,RXID(MRXTX,NTX)
 ...
 DO JS = 1,NTX                        ! Loop over transmitters
   DO JR = 1, NRXTX(JS)               ! Loop over receivers
     ...
     IF (RXVRT > .01_QL) THEN
       ...
       END DO
       I1 = 2
       CALL SET_KFG (I1,NLYR,NLYR,RXLYR,KFG)
       CALL EGT_OUT_BS (NRMGT,RHOTRP,RXLYR,KFG,NLYR,THKD,DPTHL,RMUD,SIGL,KSQL,ZR,NZ1,ZV1,NINTG, &
                        QB1R,QB1I,QB2R,QB2I,QB3R,QB3I,QB4R,QB4I,QB5R,QB5I,QB6R,QB6I)
     END IF
...
! Note that C2DINTRP uses ZS rather than ZD because MGTBS is called as a
...
! restriction ZR = 0 is used in this version.
               ...
               IF (RXLYR == NLYR) CALL EGT_OUT_DIR (XD,YD,ZD,KB,EXX,EXY,EXZ,EYY,EYZ,EZZ)
...
!  and 'source are in the same layer.
 ...
 SUBROUTINE EGT_OUT_BS (NRMGT,RHOTRP,RXLYR,KFG,NLYR,THKD,DPTHL,RMUD,SIGL,KSQL,ZR,NZ1,ZV1,NINTG, &
                       QB1R,QB1I,QB2R,QB2I,QB3R,QB3I,QB4R,QB4I,QB5R,QB5I,QB6R,QB6I)
!---------------------------------------------------------------------------------------------
...
!                  INPUT
!                  -----
!         NRMGT - number of points in RHOTRP needed for EGT_OUT computation
...
!***  Called by EGT_OUT_BOSS
!***  Calls EGT_OUT_HNK, CUBSPL

 IMPLICIT NONE
 INTEGER, PARAMETER :: QL=SELECTED_REAL_KIND(12,80)
 INTEGER NRMGT,KFG,RXLYR,NLYR,JZ,NZ1,NINTG
 REAL (KIND=QL) ZS,ZR,THKD(NLYR),DPTHL(NLYR),RMUD(0:NLYR)
 REAL RHOTRP(NRMGT),ZV1(NZ1)
 ...
 DO JZ = 1,NZ1
   ZS = REAL(ZV1(JZ),QL)
   CALL EGT_OUT_HNK (NRMGT,RHOTRP,RXLYR,KFG,NLYR,THKD,DPTHL,SIGL,KSQL,RMUD,ZR,ZS,NINTG,EHRI)
...
! Accumulate 4 integrals simultaneously until convergence of all.
 ...
 SUBROUTINE MGT_BOSS (NPLT,MXAB,NA,NB,PLAZM,PLDIP,PLUNJ,XCELL,YCELL,ZCELL,NLYR,THKD,DPTHL, &
                      RMUD,SIGL,KSQL,NTX,MRXTX,NRXTX,RXID,MXRS,NRS,XRS,YRS,ZRXTX,WTRS,     &
                      NRMGT,RHOTRP,NZ1,ZV1,NMGP,XMG,YMG,ZMG,WMG,HA,HB)
!--------------------------------------------------------------------------------------
...
!  integrated over the area of individual cells.  Units are Teslas per unit amp.
 ...
 INTEGER, PARAMETER :: QL=SELECTED_REAL_KIND(12,80)
 COMPLEX, PARAMETER :: ZERO=(0.,0.)
 INTEGER NPLT,MXAB,NLYR,RXLYR,KFG,NTX,MRXTX,NRXTX(NTX),MXRS,NRS(MRXTX,NTX),NRMGT,NZ1,NMGP, &
         JA,JB,JP,JR,JR1,JS,JAB,J1,I1,RXID(MRXTX,NTX)
 INTEGER, DIMENSION (NPLT) :: NA,NB
 REAL RHOTRP(NRMGT),ZV1(NZ1),R32(3,2),SD,CD,CP,SP,CS,SS,XS,YS,ZS,XD,YD,ZD,RHO,XBAR,YBAR, &
      MUB,RXVRT
 REAL, DIMENSION (NPLT) :: PLAZM,PLDIP,PLUNJ,WMG
 REAL, DIMENSION (MXAB,NPLT) :: XCELL,YCELL,ZCELL
 REAL, DIMENSION (MXRS,MRXTX,NTX) ::  XRS,YRS,WTRS
 ...
 REAL, DIMENSION (4,NRMGT,NZ1) :: QB1R,QB2R,QB3R,QB4R,QB5R,QB1I,QB2I,QB3I,QB4I,QB5I
 REAL, DIMENSION (NMGP,NPLT) :: XMG,YMG,ZMG
 COMPLEX(KIND=QL), DIMENSION(NLYR) :: KSQL, SIGL
 COMPLEX MGT(3,3),MGTR(3,2),QQ(5),PSI(3),KB,C2DINTRP
 COMPLEX, DIMENSION (MXAB,3,MRXTX,NTX,NPLT) :: HA,HB
 ...
 DO JS = 1,NTX                        ! Loop over transmitters
   DO JR = 1, NRXTX(JS)               ! Loop over receivers
     ...
     IF (RXVRT > .01_QL) THEN
       ...
       CALL MGTBS (NRMGT,RHOTRP,RXLYR,KFG,NLYR,THKD,DPTHL,RMUD,SIGL,KSQL,ZR,NZ1,ZV1, &
                   QB1R,QB1I,QB2R,QB2I,QB3R,QB3I,QB4R,QB4I,QB5R,QB5I)
     END IF
...
! For basement receivers, add in the direct term.  Note that C2DINTRP uses ZS
! rather than ZD because MGTBS is called as a function of ZR.  The interpolants
...
! Rotate MGT from XYZ-XYZ TO XYZ-PLT system.  Apply plate weighting for each cell.
 ...
 END SUBROUTINE MGT_BOSS
  ...
  SUBROUTINE MGTBS (NRMGT,RHOTRP,RXLYR,KFG,NLYR,THKD,DPTHL,RMUD,SIGL,KSQL,ZR, &
...
!***  Calls MGT_HNK, CUBSPL
 ...
 IMPLICIT NONE
 INTEGER, PARAMETER :: QL=SELECTED_REAL_KIND(12,80)
 INTEGER NRMGT,NZ1,NLYR,RXLYR,KFG,JZ
 ...
 DO JZ = 1,NZ1
   ZS = REAL(ZV1(JZ),QL)
   CALL MGT_HNK (NRMGT,RHOTRP,RXLYR,KFG,NLYR,THKD,DPTHL,SIGL,KSQL,RMUD,ZR,ZS,MHRI)
...
! -------------------------------------------------------------------------
 ...
 REAL, PARAMETER :: DIPL0 = 5.
 COMPLEX, PARAMETER :: ZERO = (0., 0.)
 INTEGER NTX,MXVRTX,NVRTX(NTX),NVRL,NVRLS,NRPRM,NLYR,NPLT,MXAB,NA(NPLT),NB(NPLT), &
         NZ1,JZ,JAB,JP,JS,JB,JA,JD,JV,JV1,SOURCE_TYPE,SXLYR,KFG,NDIP(MXVRTX,NTX),I1
...
!  Set depth array over which to interpolate Green's function integrals
 ...
 ALLOCATE (HLYRD(NRPRM,3,NZ1),KER(JNLO-NRPRM:JNHI,3,NZ1),QR(4,NRPRM),QI(4,NRPRM), &
           G1R(4,NRPRM,NZ1),G1I(4,NRPRM,NZ1),G2R(4,NRPRM,NZ1),G2I(4,NRPRM,NZ1),   &
           G3R(4,NRPRM,NZ1),G3I(4,NRPRM,NZ1),ZV1Q(NZ1))
 ...
 SOURCE_LOOP: DO JS = 1,NTX
   ...
   IF (ABS (ZS - ZPRV) > 1.D-4) THEN
     ...
     IF (SOURCE_TYPE == 1) THEN   ! Closed loop
       CALL PRM_HNK_CL (NRPRM,RHOTRP,NZ1,ZV1Q,ZS,NLYR,THKD,DPTHL,RMUD,SIGL,KSQL,KFG,SXLYR,KER,HLYRD)
     ELSE                         ! Open loop
       CALL PRM_HNK_OL (NRPRM,RHOTRP,NZ1,ZV1Q,ZS,NLYR,THKD,DPTHL,RMUD,SIGL,KSQL,KFG,SXLYR,KER,HLYRD)
     END IF
...
!  Step through depths.  Add up contributions for each dipole for each transmitter.
 ...
 DEALLOCATE (HLYRD,KER,QR,QI,ZV1Q,G1R,G1I,G2R,G2I,G3R,G3I)
...
! Compute layered-earth coefficients.  RXLYR = NLYR
 ...
 SUBROUTINE PRM_BOSS_MT (NLYR,THKD,DPTHL,RMUD,KSQL,NPLT,PLAZM,PLDIP,PLUNJ,MXAB,NB,NA, &
                         ZCELL,NZ1,ZV1,E_PRYM)
! ----------------------------------------------------------------------------------
...
!  Computes the primary electric field, E_PRYM, due to two plane wave orientations,
!  one North (X) and the other East (Y) in NPLT plates lying in the basement of a host
!  with NLYR-1 layers above basement.  R23 is used to rotate the fields into the
!  components tangential to each plate.
!
...
!          ------
!
!  E_PRYM(1,*,1,*) - complex primary electric fields along strike - X polarisation.
!  E_PRYM(2,*,1,*) - complex primary electric fields down-dip     - X polarisation..
!
!  E_PRYM(1,*,2,*) - complex primary electric fields along strike - Y polarisation.
!  E_PRYM(2,*,2,*) - complex primary electric fields down-dip     - Y polarisation..
!
!    The first NAB components of E_PRYM are the fields along strike and
!    the second group of NA*NB components are along dip.  Thus for a strike
!    along the X axis, the first NA*NB components would be the X component
...
! -------------------------------------------------------------------------
 ...
 IMPLICIT NONE
 INTEGER, PARAMETER :: QL=SELECTED_REAL_KIND(12,80), NTX=2  ! 2 polarisation directions => NTX = 2
 COMPLEX, PARAMETER :: ZERO = (0., 0.)
 INTEGER RXLYR,NLYR,NPLT,MXAB,NA(NPLT),NB(NPLT),NBL,NAL,JAB,JP,JB,JA,JZ,NZ1
 REAL, DIMENSION(4,NZ1) :: FXXR,FXXI
 REAL DPTHN,SAZM,CAZM,CDIP,SDIP,CPLN,SPLN,ZCELL(MXAB,NPLT),ZV1(NZ1),R23(2,3)
 REAL, DIMENSION (NPLT) :: PLAZM,PLDIP,PLUNJ
 REAL(KIND=QL) RMUD(0:NLYR),ZR
 REAL(KIND=QL), DIMENSION(NLYR) :: THKD,DPTHL
 COMPLEX KPN,E_PRYM(2,MXAB,NTX,NPLT),FXX,EXYZ(3),EAB(2)
 COMPLEX(KIND=QL) KSQL(NLYR),KPL(NLYR),FXXQ,FXX1,P,Q
 ...
 DO JZ = 1,NZ1
   FXXQ = Q * EXP (KPN * (DPTHN - ZV1(JZ)))
   FXX = CMPLX (FXXQ)
   FXXR(1,JZ) = REAL (FXX)
   FXXI(1,JZ) = AIMAG (FXX)
 END DO

 CALL CUBSPL (ZV1,FXXR,NZ1)
 CALL CUBSPL (ZV1,FXXI,NZ1)
...
!  transmitter.
   ...
   DEPTH_STEP: DO JB = 1, NBL
     STRYK_STEP: DO JA = 1, NAL
       JAB = JA + (JB-1)*NAL
       ZR = REAL (ZCELL (JAB,JP),QL)
       CALL CDCUBVAL (ZV1,FXXR,FXXI,NZ1,ZR,FXX1)

       EXYZ = ZERO
       EXYZ(1) = CMPLX (FXX1)               ! X-polarised electric field
       EAB = MATMUL (R23, EXYZ)
       E_PRYM(1:2,JAB,1,JP) = EAB(1:2)

       EXYZ = ZERO
       EXYZ(2) = CMPLX (FXX1)               ! Y-polarised electric field
       EAB = MATMUL (R23, EXYZ)
       E_PRYM(1:2,JAB,2,JP) = EAB(1:2)
     END DO STRYK_STEP
   END DO DEPTH_STEP
 END DO PLATE_LOOP
...
! -------------------------------------------------------------------------
 ...
 COMPLEX, PARAMETER :: ONE=(1.,0.)
 INTEGER NTX,NRPRM,NLYR,NPLT,MXAB,NA(NPLT),NB(NPLT),NBL,NAL,JAB,NINTG, &
 ...
 COMPLEX(KIND=QL), DIMENSION (NLYR) :: SIGL,KSQL
 COMPLEX(KIND=QL), DIMENSION(:,:,:), ALLOCATABLE :: FXX,KER
 LOGICAL TILT
...
!  Set depth array over which to interpolate Green's function integrals
 ...
 ALLOCATE (FXX(NRPRM,5,NZ1),KER(JNLO-NRPRM:JNHI,5,NZ1),ZV1Q(NZ1),               &
           G1R(4,NRPRM,NZ1),G1I(4,NRPRM,NZ1),G2R(4,NRPRM,NZ1),G2I(4,NRPRM,NZ1), &
           G3R(4,NRPRM,NZ1),G3I(4,NRPRM,NZ1),G4R(4,NRPRM,NZ1),G4I(4,NRPRM,NZ1), &
           G5R(4,NRPRM,NZ1),G5I(4,NRPRM,NZ1))
...
!  of horizontal distance and depth.  Step through different altitude levels.
 ...
 SOURCE_LOOP: DO JS = 1,NTX
   ...
   FXX = ZERO; KER = ZERO
   CALL PRM_HNK_MD (NRPRM,RHOTRP,NZ1,ZV1Q,ZS,NLYR,THKD,DPTHL,RMUD,SIGL,KSQL,KFG,RXLYR,SXLYR,NINTG,KER,FXX)
...
!  RXLYR > 0 since target cannot be in air.
 ...
 DEALLOCATE (FXX,KER,ZV1Q,G1R,G2R,G3R,G4R,G5R,G1I,G2I,G3I,G4I,G5I)
...
!  CP, SP - cosine and sine of plate rotation
 ...
 SUBROUTINE SET_CELLS (NW,DPTHB,NPLT,MXAB,CELLW,PLNGTH,PLWDTH,XCNTR,YCNTR,PLTOP,PLAZM,PLDIP, &
                       PLUNJ,NA,NB,DA,DB,MXCL2,NCELL2,XCELL,YCELL,ZCELL,ZMIN,ZMAX,SHIFT)
!------------------------------------------------------------------------------------------

!***  Called by SHOW_AND_TELL, LEROI
!***  Calls RPLT2XYZ

!  This routine sets up the (X,Y,Z) coordinates of the plate cell centres,
!  establishes vertical boundaries for interpolation and shifts the whole plate
!  structure downwards if any of it would otherwise intrude above basement
!
!  The local 2D coordinate system of each plate is defined by XI being the
!  distance along the strike edge and ETA the distance down the dip edge.
!
...
!
!
!  In the transformed scheme, the cells are numbered from south to north,
!  (positive XI pointing north) starting from the west most column and then to subsequent
!  columns (positive ETA)
!
...
!
!  In this version of Leroi, the plate is not allowed to rise out of basement.
!  Thus a check is made to see if any corners protrude above basement.  If so
!  that plate is shifted downwards by the maximum protrusion.
!
!        Input variables
!        ---------------
!
!   NW        - unit number for Leroi.out
!   DPTHB     - upper limit for plates
!   NPLT      - Number of plates
!   MXAB      - Number of cells in biggest plate
!   CELLW     - nominal cell dimension
!   PLNGTH    - strike length for each plate
...
!   MXCL2       - 2 * total number of cells in all plates = NCELL2(NPLT)
 ...
 IMPLICIT NONE
 INTEGER NW,MXAB,MXCL2,NPLT,NCELL2(0:NPLT),JP,JA,JB,JAB
 INTEGER, DIMENSION(NPLT) :: NA,NB
 REAL CELLW,P_LIMIT,PL2,XI0,ETA0,P(2),XYZ(3),R32(3,2),DPTHB, &
      CA,SA,CP,SP,CD,SD,ZMINP,ZMIN,ZMAX,TMPL
 REAL, DIMENSION(4) :: XCNR,YCNR,ZCNR
 REAL, DIMENSION(NPLT) :: PLNGTH, PLWDTH,XCNTR,YCNTR,PLTOP,SHIFT,PLAZM,PLUNJ,PLDIP,DA,DB
 REAL, DIMENSION(MXAB,NPLT) :: XCELL,YCELL,ZCELL

!  Lay out inoitial cell pattern in coordinate system centred 1t (XCNTR, YCNTR, PLTOP)
!  Compute corner coordinates as well.

 P_LIMIT = DPTHB + 0.01                 ! Upper limit for plate corners

 XCELL = 0. ; YCELL = 0. ; ZCELL = 0.;  NCELL2 = 0
 ZMIN = 1.E6;  ZMAX = -1.E6
 ...
 DO JP = 1, NPLT      !  Sum over plates
   ...
   XCNR(2:3) = NA(JP) * DA(JP) / 2.
   XCNR(1) = -XCNR(2);  XCNR(4) = XCNR(1)
   YCNR(1:2) = 0.
   YCNR(3:4) = NB(JP) * DB(JP)
   ...
   IF (ZMINP < P_LIMIT) THEN
     SHIFT(JP) = P_LIMIT - ZMINP
 ...
 SUBROUTINE SCAT_MTRX_BOSS (NLYR,THKD,DPTHB,RMUD,SIGL,KSQL,NPLT,MXAB,NCELL2,MXCL2,NA,NB, &
                            DA,DB,SIGT,KSQT,PLTOP,PLAZM,PLDIP,PLUNJ,XCELL,YCELL,ZCELL,   &
                            NZ2,ZV2,NREGT,RHOTRP,NTX,E_PRYM,J_SCAT,DCMP_FAIL)
!--------------------------------------------------------------------------------------
...
!           INDX - pivoting vector information
 ...
 INTEGER, PARAMETER :: QL=SELECTED_REAL_KIND(12,80)
 COMPLEX, PARAMETER :: ZERO=(0.,0.)
 INTEGER NTX,NLYR,NPLT,MXCL2,MCELL2,NA(NPLT),NB(NPLT),NCELL2(0:NPLT),MXAB,MXA,MXB, &
         NAL,NBL,NAB,IPHI(4),IPSI(4),IDX(4),INDX(MXCL2),KB,JB,KA,JA,JZ,L,NREGT,JP,  &
         JPS,JPR,JSX,JRX,JSXRX,KAB,KABG,JAB,NZ2
 REAL AD(4),BD(4),GA,GB,DAL,DBL,DPTHB,PTOPL,CAZM,SAZM,CDP,SDP,CPL,SPL,RHOTRP(NREGT), &
      ZTR,ZV2(NZ2),XCELL(MXAB,NPLT),YCELL(MXAB,NPLT),ZCELL(MXAB,NPLT),AREA
 REAL(KIND=QL) RMUD(0:NLYR),THKD(NLYR)
 REAL, DIMENSION (NPLT) :: DA,DB,PLTOP,PLAZM,PLDIP,PLUNJ
 REAL, DIMENSION(4,NREGT) :: GR1,GI1,GR2,GI2,GR3,GI3,GR4,GI4,GR5,GI5,GR6,GI6
 REAL, DIMENSION(4,NREGT,NZ2) :: GR1Z,GI1Z,GR2Z,GI2Z,GR3Z,GI3Z,GR4Z,GI4Z,GR5Z,GI5Z,GR6Z,GI6Z
 REAL, ALLOCATABLE, DIMENSION(:) :: XCEL1,YCEL1,ZCEL1
 COMPLEX KSQN,KBASE,SCAT_MTRX(MXCL2,MXCL2),E_PRYM(2,MXAB,NTX,NPLT),J_SCAT(2,MXAB,NTX,NPLT), &
...
! row and column indices of the "source" cells
             ...
             KAB_L: DO KAB = 1, NA(JSX) * NB(JSX)    !  "source" cell index
               ...
               IF (JSXRX == 1) THEN
                 ...
                 ABSS = SABI(JAB,KAB) * AREA
                 BAS  = SBAI(JAB,KAB) * AREA
                 BBS  = SBBI(JAB,KAB) * AREA
                 AAV  = VAAI(JAB,KAB) * AREA
                 ABV  = VABI(JAB,KAB) * AREA
                 BAV  = VBAI(JAB,KAB) * AREA
                 BBV  = VBBI(JAB,KAB) * AREA
               ELSE
                 AAS  = SAAI(KAB,JAB) * AREA
                 ABSS = SBAI(KAB,JAB) * AREA
                 BAS  = SABI(KAB,JAB) * AREA
                 BBS  = SBBI(KAB,JAB) * AREA
                 AAV  = VAAI(KAB,JAB) * AREA
                 ABV  = VBAI(KAB,JAB) * AREA
                 BAV  = VABI(KAB,JAB) * AREA
                 BBV  = VBBI(KAB,JAB) * AREA
               END IF
...
!        INDX - pivoting vector information from SCAT_MTRX_LU_DCMP
 ...
 SUBROUTINE SCAT_EM (JF,NFRQ,NPLT,MXAB,NA,NB,PLAZM,PLDIP,PLUNJ,XCELL,YCELL,ZCELL,NMGP,XMG,YMG,ZMG,WMG, &
                     NLYR,THKD,DPTHL,RMUD,SIGL,KSQL,NTX,MRXTX,NRXTX,RXID,MXRS,NRS,XRS,YRS,ZRXTX,WTRS,  &
                     EDCS,EDSN,NRMGT,RHOTRP,NZ1,ZV1,MDRX,EDRX,J_SCAT,BFD_SCAT,QUIT)
!-----------------------------------------------------------------------------------------------------
!
!***  Called by LEROI_3D
!***  Calls MGT_BOSS, EGT_OUT_BOSS
!
...
!       NRMGT - Dimension for MGT interpolations
!      RHOTRP - horizontal interpolation array for MGT & EGT_OUT
!    ZV1(NZ1) - vertical interpolatin array
...
!       NRMGT - Dimension for receiver Green's function interpolations
 ...
 INTEGER, PARAMETER :: QL=SELECTED_REAL_KIND(12,80)
 COMPLEX, PARAMETER :: ZERO=(0.,0.)
 INTEGER JF,NFRQ,JP,NPLT,MXAB,NLYR,NTX,MRXTX,NRXTX(NTX),RXID(MRXTX,NTX),MXRS, &
         NRS(MRXTX,NTX),NRMGT,NZ1,NMGP,JS,JR,JC,JCL
 INTEGER, DIMENSION (NPLT) :: NA,NB
 REAL RHOTRP(NRMGT),ZV1(NZ1),EDCS(MRXTX,NTX),EDSN(MRXTX,NTX),SCRATCH(MRXTX,NTX,3), &
      WTRS(MXRS,MRXTX,NTX), ZRXTX(MRXTX,NTX)
...
!  them with the scattering currents and sum.
 ...
 IF (MDRX) CALL MGT_BOSS (NPLT,MXAB,NA,NB,PLAZM,PLDIP,PLUNJ,XCELL,YCELL,ZCELL,NLYR,THKD,DPTHL, &
                          RMUD,SIGL,KSQL,NTX,MRXTX,NRXTX,RXID,MXRS,NRS,XRS,YRS,ZRXTX,WTRS,     &
                          NRMGT,RHOTRP,NZ1,ZV1,NMGP,XMG,YMG,ZMG,WMG,HA,HB)

 IF (EDRX) CALL EGT_OUT_BOSS (NPLT,MXAB,NA,NB,PLAZM,PLDIP,PLUNJ,XCELL,YCELL,ZCELL,NLYR,THKD,DPTHL, &
                             RMUD,SIGL,KSQL,NTX,MRXTX,NRXTX,RXID,MXRS,NRS,XRS,YRS,ZRXTX,WTRS,    &
                             NRMGT,RHOTRP,NZ1,ZV1,NMGP,XMG,YMG,ZMG,WMG,HA,HB)
...
! For coincident loop receivers, put the response into component 1
 ...
 SUBROUTINE SET_MAX_INTRP (MT,MXRHO,RHOTRP,MXAB,NPLT,NA,NB,DA,PLNGTH,XCELL,YCELL,NTX,MXVRTX, &
                           NVRTX,SXN,SXE,MRXTX,NRXTX,MXRS,NRS,XRS,YRS,NRMGT,NRPRM,NREGT)
!---------------------------------------------------------------------------------------------
...
!              = 4 : coincident loop
!              = 5 : plane wave
!  MT          : true only for magnetotellurics inversion
!
!
...
!
 ...
 IMPLICIT NONE
 INTEGER, PARAMETER :: IP=1, QL=SELECTED_REAL_KIND(12,80), MAXITS_INT=5
 REAL, PARAMETER :: BND=0.01, EXPND=2., RSVT0=0.1, ETA=1.E-7, TOL=.5E-31,GCRIT=1.E-4
 INTEGER NPAR,NDATA,MAXITS,ITS,ICNT,NW,NW1,INVPRT,CXPAR(NPAR),  &
         ...
         NRXTX(MXTX),RXID(MRXTX,NTX),MQVR,MXRS,LNTR(4,NLINES),MD1,MD2,IPM,NERX,NHRX,  &
         IDHE(NERX),MXRHO,KNORM2(MRXTX,NTX),JP,NCTD(MRXTX,NTX)
 ...
 REAL(KIND=QL) NCNTRD,ECNTRD,YXZPLT(3,MRXL,NLINES),MTPLT(2,NERX)
 LOGICAL,PARAMETER :: WITHU=.TRUE., WITHV=.TRUE.
 LOGICAL JCBN,FINAL,MT,MOVE
 CHARACTER (LEN=120) TITLE
...
!----------------------------------------------------------------------
 ...
 ITER_LOOP: DO ITS = 1, MAXITS
   ...
   CALL FORJAC (NDATA,NPAR,NLINES,MLINES,MCMP,MRXL,MCHNL,NCHNL,NFRQ,NFRQHS,TDFD,STEP,FREQ, &
                ...
                SVAZM,UNITS,RX_TYPE,SOURCE_TYPE,IDH,MT,IPM,IDHE,MXTX,NTX,MXVRTX,NVRTX,SXN, &
                SXE,SXZ,SXDIP,SXAZM,NRX,LNTR,NRXTX,MRXTX,RXID,MQVR,MXRS,XRXTX,YRXTX,ZRXTX, &
                MD1,MD2,RXAZM,RXDIP,NERX,NHRX,KNORM2,NCTD,MXRHO,RHOTRP,NLYR,RMUD,REPS,     &
                CHRG,CTAU,CFREQ,NPLT,XYNORM,CELLW,MXAB,CHRGP,CTAUP,CFREQP,PARFAC,XPAR,     &
                CXPAR,BPRM,RWTS,XDATA,RMODL,XMODL,SUMSQ,JCBN,A,VERR,NW)
   ...
   CALL INDEX_MPAR
   IF (ITS == 1) THEN
     WRITE(NW1,28) RMSERR,RSVT
     WRITE(NW1,29) MPAR(1:NPAR)
   END IF
...
!  and try again.  Give up and return after MAXITS_INT "internal" iterations.
     ...
     CALL FORJAC (NDATA,NPAR,NLINES,MLINES,MCMP,MRXL,MCHNL,NCHNL,NFRQ,NFRQHS,TDFD,STEP,FREQ, &
                  ...
                  SXE,SXZ,SXDIP,SXAZM,NRX,LNTR,NRXTX,MRXTX,RXID,MQVR,MXRS,XRXTX,YRXTX,ZRXTX, &
                  MD1,MD2,RXAZM,RXDIP,NERX,NHRX,KNORM2,NCTD,MXRHO,RHOTRP,NLYR,RMUD,REPS,     &
                  CHRG,CTAU,CFREQ,NPLT,XYNORM,CELLW,MXAB,CHRGP,CTAUP,CFREQP,PARFAC,GXPAR,    &
                  CXPAR,BPRM,RWTS,XDATA,RMODL,XMODL,SSQNEW,JCBN,A,VERR,NW)
...
!*******************************************
   ...
   IF (INVPRT == 3) CALL WRITE_DATA
   CLOSE (NW1)                                                            ! Flush Buffer
   OPEN(NW1,FILE = 'Leroi.mv1',STATUS = 'OLD', POSITION = 'APPEND')
 ...
 CALL WRITE_INVMDL (NW,FINAL,ITS,NLYR,NPLT,NPAR,XPAR,XYNORM,NCNTRD,ECNTRD)
 WRITE(NW1,32) ITS,RMSERR,NSR,RSVT
 CALL INDEX_MPAR
...
!  PLTOP is negative downwards and plate north, east location is as specified.
 ...
 DO J2 = 1,NPLT
   ...
   MPAR(KP+2) = -EXP (XPAR(KP+2))            ! PLTOP(J2) in RL
   MPAR(KP+3) = EXP (XPAR(KP+3))                      ! PLNGTH(J2)
   MPAR(KP+4) = EXP (XPAR(KP+4))                      ! PLWDTH(J2)
...
!  ---------------------
   ...
   IF (MT) THEN
     CALL WRITE_MT (NW,NW1,KPRT,NFRQ,FREQ,NERX,TITLE,MTPLT,IPM,RMODL,RDATA,RWTS)
   ELSE IF (TDFD == 2) THEN
     CALL WRITE_FD (NW,NW1,KPRT,NFRQ,MCHNL,NLINES,MRXL,MCMP,NRX,SURVEY_TYPE,LINE,IDH,RX_TYPE,UNITS, &
...
!       CURNT : current
!        ISYS = 2 for Sampo;  4 for UTEM;  6 for MT;  0 otherwise
!
!             Survey Input Variables
!             ----------------------
!
!        NLINES : number of lines of data
!          MCMP = 1 for Sampo and coincident loop, 4 for MT, 3 otherwise
...
!          IDHE : identifies which magnetic receiver used to compute impedance for each ED
...
!     NERX,NHRX : number of electric and magnetic stations respectively for MT
...
!   PLAZM, PLDIP & PLUNJ are normalised to PI
 ...
 INTEGER NDATA,NPAR,NLINES,MCMP,MRXL,MCHNL,NCHNL,NFRQ,NFRQHS,NFT,TDFD,STEP,NSX,NPULS,NTYPLS, &
         ...
         RXID(MRXTX,NTX),MQVR,MXRS,LNTR(4,NLINES),MD1,MD2,IPM,NERX,NHRX,IDHE(NERX),    &
         KNORM2(MRXTX,NTX),MXRHO,CXPAR(NPAR),MLINES,JC,JR,JD,JP,JL,LP,JF,K0,NW
 INTEGER, DIMENSION(NLINES) :: RX_TYPE,UNITS,NRX,IDH
 INTEGER RWTS(MCHNL,MRXL,MCMP,MLINES)
 REAL FREQ(NFRQ),FRQHS(NFRQHS),SWX(NSX),SWY(NSX,3),PULSE,TRP(NTYRP),TOPN(NCHNL),TCLS(NCHNL),   &
      CURNT(NFT),XRXTX(MRXTX,NTX,MQVR),YRXTX(MRXTX,NTX,MQVR),ZRXTX(MRXTX,NTX),SXN(MXVRTX,NTX), &
      SXE(MXVRTX,NTX),SXZ(MXTX),SXDIP(MXTX),SXAZM(MXTX),RHOTRP(MXRHO),XPAR(NPAR),SUMSQ,  &
      A(NDATA,NPAR+1),BTD(NCHNL,MRXTX,NTX,3),BTD_SCAT(NCHNL,MRXTX,NTX,3),BPRM(MRXTX,NTX),      &
      SVAZM(NLINES),VM,VJ,VD,DENOM,V1,DELTA,DELXY,DELZ,XP0,PARFAC,RMU1,DELPHI
...
!             PARFAC and Jacobian column LP isconstructed.
 ...
 ELSE IF (TDFD == 2) THEN  !  Construct the frequency-domain response (except for MT).
...
IF (FINAL) THEN
 ...
 DO JP = 1,NPLT
   K0 = 9*(JP-1)
   PLTE = 0.
   DO J = 1,4   ! SIG_T, PLTOP, PLNGTH, PLWDTH
...
!    Writing order: SIG_T, PLTOP, ECNTTR, NCNTR,  PLNGTH, PLWDTH, PLAZM, PLDIP, PLUNJ
   ...
   WRITE(*,7)  JP,PLTE(1),ZP,DPLTE(1:2),PLTE(3:4),ANG(1:3)
   WRITE(NW,7) JP,PLTE(1),ZP,DPLTE(1:2),PLTE(3:4),ANG(1:3)
 END DO
